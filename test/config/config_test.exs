defmodule FormFlow.ConfigTest do
  ...
  defmodule Gated do
    ...
    @impl true
    def handle_instance_mount(%Context{user_id: "stranger"}, _config_data),
      do: {:error, "This flow is not yours."}

    def handle_instance_mount(%Context{}, %{greeting: greeting}), do: {:ok, %{greeting: greeting}}
    def handle_instance_mount(%Context{}, _config_data), do: {:ok, %{}}
  end
  ...
  describe "handle_instance_mount/2" do
    ...
    test "a custom config inherits the type lists and overrides only what it changes" do
      ...
      assert Gated.handle_instance_mount(%Context{user_id: "stranger"}, %{}) ==
               {:error, "This flow is not yours."}

      assert Gated.handle_instance_mount(%Context{user_id: "owner"}, %{greeting: "hi"}) ==
               {:ok, %{greeting: "hi"}}
    end
  end

  describe "Shared.handle_instance_mount/2 applies a config's answer to the page" do
    test "a refusal lands in :mount_error and skips the continuation" do
      socket =
        Shared.handle_instance_mount(socket(Gated), fn _socket -> flunk("started anyway") end)
    ...
    test "no config means the library's default, which allows" do
      socket =
        Shared.handle_instance_mount(socket(nil), &Phoenix.Component.assign(&1, :started?, true))

      assert socket.assigns.started? == true
    end

    test "a malformed answer fails closed, naming the module" do
      assert_raise ArgumentError, ~r/Broken.handle_instance_mount\/2 returned :whatever/, fn ->
        Shared.handle_instance_mount(socket(Broken))
      end
    end
  end
end

...
# Every route is declared: nothing falls through to a catch-all. The two
# `*path` mounts are FormFlow.Web.Router's, which dispatches the segments it is
# handed to the library's LiveComponents — /demo/admin/* for templates,
# /demo/pet-licenses/applications/* for a user's own flows, and
# /demo/pet-licenses/reviews/* for the reviewer's. The live_session's on_mount hook assigns the demo's
...
#
# It also includes priv/repo/form_flow_snapshot.sql and the migration that
# replays it, so the fresh database starts with the pet licensing flows built
# by hand in the admin UI. snapshot.sh rewrites the SQL from the running
# demo's database.
